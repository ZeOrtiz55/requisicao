...
export default function CardReq({ req, onUpdate, onPrint }: { req: any, onUpdate: any, onPrint: any }) {
  ...
  // FUNÇÃO ALTERADA: REMOVE O CARD E REORGANIZA A LISTA
  const removerCotacao = (idx: number) => {
    if (confirm(`Remover o Fornecedor ${idx} desta lista?`)) {
        let newData = { ...cotacaoData };
        
        // Desloca os dados dos fornecedores seguintes para cima
        for (let j = idx; j < 5; j++) {
        ...
        // Limpa o último fornecedor da memória após o deslocamento
        newData[`fornecedor5`] = '';
        ...
        // Diminui a quantidade de cards visíveis (mínimo de 1)
                ...
                return (
                  <div key={idx} className={`${bentoStyle} border-l-[8px] border-l-blue-500 animate-in slide-in-from-right-4 duration-300 relative`}>
                    <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                      ...
                      {/* BOTÃO X PARA REMOVER O CARD COMPLETAMENTE */}
                      <button 
